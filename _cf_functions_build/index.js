...
var init_transform = __esm({
  "api/mainsite/ai/transform.ts"() {
    "use strict";
    init_functionsRoutes_0_834891547028554();
    GEMINI_CONFIG = {
...
${text}`;
        ...
        return new Response(JSON.stringify({ error: error instanceof Error ? error.message : "Erro desconhecido na gera\xE7\xE3o por IA." }), { status: 500 });
      }
    }, "onRequestPost");
  }
});
...
var init_hub_config = __esm({
  ...
  `).all();
    ...
    resolveHubConfig = /* @__PURE__ */ __name(async (env, module) => {
...
async function onRequestGet2(context) {
  ...
  try {
    const resolved = await resolveHubConfig(context.env, "adminhub");
    ...
    const message = error instanceof Error ? error.message : "Falha ao carregar configura\xE7\xE3o do adminhub";
    await logHubEvent(context.env.BIGDATA_DB, {
      module: "adminhub",
      action: "config-read",
      source: "bigdata_db",
...
async function onRequestPut(context) {
  ...
  if (!context.env.BIGDATA_DB) {
    return buildErrorResponse("BIGDATA_DB n\xE3o configurado no runtime.", trace, 503);
  }
  try {
    const body = await context.request.json();
    const adminActor = resolveAdminActorFromRequest(context.request, body);
    const cards = parseCardsFromBody(body);
    const updated = await saveCardsToDb(context.env.BIGDATA_DB, "adminhub", cards, adminActor);
    await logHubEvent(context.env.BIGDATA_DB, {
      module: "adminhub",
      action: "config-save",
      source: "bigdata_db",
    ...
    const message = error instanceof Error ? error.message : "Falha ao salvar configura\xE7\xE3o do adminhub";
    await logHubEvent(context.env.BIGDATA_DB, {
      module: "adminhub",
      action: "config-save",
      source: "bigdata_db",
...
var init_config = __esm({
  "api/adminhub/config.ts"() {
    ...
    init_request_trace();
    init_auth();
    buildErrorResponse = /* @__PURE__ */ __name((message, trace, status = 500) => new Response(JSON.stringify({
      ok: false,
      error: message,
      ...trace
    }), {
...
async function onRequestGet3(context) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Falha ao carregar configura\xE7\xE3o do apphub";
    await logHubEvent(context.env.BIGDATA_DB, {
      module: "apphub",
      action: "config-read",
      source: "bigdata_db",
    ...
    return buildErrorResponse2(message, trace, 500);
  }
}
async function onRequestPut2(context) {
  ...
  if (!context.env.BIGDATA_DB) {
    return buildErrorResponse2("BIGDATA_DB n\xE3o configurado no runtime.", trace, 503);
  }
  try {
    const body = await context.request.json();
    const adminActor = resolveAdminActorFromRequest(context.request, body);
    ...
    const message = error instanceof Error ? error.message : "Falha ao salvar configura\xE7\xE3o do apphub";
    await logHubEvent(context.env.BIGDATA_DB, {
      module: "apphub",
      action: "config-save",
      source: "bigdata_db",
    ...
    return buildErrorResponse2(message, trace, 400);
  }
}
...
var init_config2 = __esm({
  "api/apphub/config.ts"() {
    ...
    init_request_trace();
    init_auth();
    buildErrorResponse2 = /* @__PURE__ */ __name((message, trace, status = 500) => new Response(JSON.stringify({
      ok: false,
      error: message,
      ...trace
    }), {
...
async function onRequestPost2(context) {
  ...
  try {
    ...
    if (!isValidEmail(emailDestino)) {
      return json({ ok: false, error: "E-mail de destino inv\xE1lido.", ...trace }, 400);
    }
    if (!relatorioHtml && !relatorioTexto) {
      return json({ ok: false, error: "Relat\xF3rio vazio.", ...trace }, 400);
    }
    if (!env.RESEND_API_KEY) {
      return json({ ok: false, error: "RESEND_API_KEY n\xE3o configurada no runtime.", ...trace }, 503);
    }
    ...
    const message = error instanceof Error ? error.message : "Falha interna na comunica\xE7\xE3o do e-mail.";
    if (env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(env.BIGDATA_DB, {
          module: "astrologo",
...
async function onRequestPost3(context) {
  ...
  if (!db) {
    return json2({ ok: false, error: "Nenhum binding D1 dispon\xEDvel (BIGDATA_DB).", ...trace }, 503);
  }
  try {
    const body = await context.request.json();
    const adminActor = resolveAdminActorFromRequest(context.request, body);
    ...
    if (!id) {
      return json2({ ok: false, error: "ID inv\xE1lido.", ...trace }, 400);
    }
    ...
    return json2({ ok: true, id, admin_actor: adminActor, ...trace });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Falha ao excluir mapa do Astr\xF3logo";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "astrologo",
    ...
    return json2({ ok: false, error: message, ...trace }, 500);
  }
}
...
var init_excluir = __esm({
  "api/astrologo/excluir.ts"() {
    ...
    resolveDb = /* @__PURE__ */ __name((context) => context.env.BIGDATA_DB, "resolveDb");
...
async function onRequestPost4(context) {
  const trace = createResponseTrace(context.request);
  const db = resolveDb2(context);
  const source = resolveOperationalSource2();
  if (!db) {
    return json3({ ok: false, error: "Nenhum binding D1 dispon\xEDvel (BIGDATA_DB).", ...trace }, 503);
  }
  try {
    const body = await context.request.json();
    const adminActor = resolveAdminActorFromRequest(context.request, body);
    const id = String(body.id ?? "").trim();
    if (!id) {
      return json3({ ok: false, error: "ID inv\xE1lido.", ...trace }, 400);
    }
    ...
    `).bind(id).first();
    if (!mapa) {
      return json3({ ok: false, error: "Mapa n\xE3o encontrado.", ...trace }, 404);
    }
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
    ...
    const message = error instanceof Error ? error.message : "Falha ao ler mapa do Astr\xF3logo";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "astrologo",
    ...
    return json3({ ok: false, error: message, ...trace }, 500);
  }
}
...
var init_ler = __esm({
  "api/astrologo/ler.ts"() {
    ...
    resolveDb2 = /* @__PURE__ */ __name((context) => context.env.BIGDATA_DB, "resolveDb");
...
async function onRequestGet5(context) {
  ...
  const source = resolveOperationalSource3(context);
  if (!db) {
    return json4({ ok: false, error: "Nenhum binding D1 dispon\xEDvel (BIGDATA_DB).", ...trace }, 503);
  }
  try {
    ...
    return json4({ ok: true, policies, admin_actor: adminActor, ...trace });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Falha ao carregar painel de rate limit do Astr\xF3logo";
...
async function onRequestPost5(context) {
  ...
  const source = resolveOperationalSource3(context);
  if (!db) {
    return json4({ ok: false, error: "Nenhum binding D1 dispon\xEDvel (ASTROLOGO_SOURCE_DB/BIGDATA_DB).", ...trace }, 503);
  }
  try {
    const body = await context.request.json();
    const adminActor = resolveAdminActorFromRequest(context.request, body);
    ...
    if (!route) {
      return json4({ ok: false, error: "Rota de rate limit inv\xE1lida.", ...trace }, 400);
    }
    ...
    if (action === "restore_default") {
      ...
      return json4({ ok: true, action: "restore_default", policies: policies2, admin_actor: adminActor, ...trace });
    }
    ...
    if (maxRequests > 500 || windowMinutes > 1440) {
      return json4({ ok: false, error: "Par\xE2metros fora da faixa permitida.", ...trace }, 400);
    }
    await upsertRateLimitPolicy(db, {
      route,
      enabled,
    ...
    return json4({ ok: true, action: "update", policies, admin_actor: adminActor, ...trace });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Falha ao salvar painel de rate limit do Astr\xF3logo";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "astrologo",
    ...
    return json4({ ok: false, error: message, ...trace }, 500);
  }
}
var json4, normalizeRoute, toPositiveInt, buildFallbackPolicies, resolveRateLimitDb, resolveOperationalSource3;
var init_rate_limit = __esm({
  "api/astrologo/rate-limit.ts"() {
    ...
    init_admin_actor();
    init_request_trace();
    json4 = /* @__PURE__ */ __name((data, status = 200) => new Response(JSON.stringify(data), {
      status,
      headers: toHeaders()
    }), "json");
    ...
    resolveRateLimitDb = /* @__PURE__ */ __name((context) => context.env.BIGDATA_DB, "resolveRateLimitDb");
...
async function onRequestPost6(context) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Falha inesperada no sync do Astr\xF3logo";
    await finishSyncRun(env.BIGDATA_DB, {
      id: syncRunId,
      status: "error",
      finishedAt: Date.now(),
...
var FINANCIAL_CUTOFF_BRT, FINANCIAL_CUTOFF_DATE, FINANCIAL_CUTOFF_UTC, FINANCIAL_CUTOFF_ISO, getStartIsoWithCutoff, isOnOrAfterCutoff, onRequestGet7;
...
var FINANCIAL_CUTOFF_DB_UTC, getStartDbWithCutoff, onRequestGet8;
...
var import_mercadopago, onRequestPost7;
...
var import_mercadopago2, onRequestPost8;
...
var import_mercadopago3, FINANCIAL_CUTOFF_BRT2, FINANCIAL_CUTOFF_UTC2, FINANCIAL_CUTOFF_ISO2, FINANCIAL_CUTOFF_DB_UTC2, onRequestPost9;
...
var normalizeSumupStatus, normalizeMPStatus, onRequestPost10;
...
var FINANCIAL_CUTOFF_DB_UTC3, getStartDbWithCutoff2, onRequestGet9;
...
var FINANCIAL_CUTOFF_BRT3, FINANCIAL_CUTOFF_UTC3, isOnOrAfterCutoff2, normalizeSumupStatus2, onRequestPost11;
    ...
    onRequestPost11 = /* @__PURE__ */ __name(async (context) => {
      ...
      try {
        ...
        for (const checkout of checkouts) {
          ...
          const status = normalizeSumupStatus2(tx?.status || checkout.status || "UNKNOWN");
...
var DEFAULT_PARAMS, DEFAULT_POLICIES2, SUPPORTED_ROUTES2, toHeaders3, toInt2, toRate, validateRate, ensureParametrosTables, readLatestParams, ensureRateLimitTables2, ensureDefaultPolicies2, getRateLimitWindowStats2, listPoliciesWithStats2, upsertRateLimitPolicy2, resetRateLimitPolicy2;
...
async function onRequestGet11(context) {
  ...
  const source = resolveOperationalSource4(context);
  if (!db) {
    return json5({ ok: false, error: "Nenhum binding D1 dispon\xEDvel (BIGDATA_DB).", ...trace }, 503);
  }
  try {
    ...
    const message = error instanceof Error ? error.message : "Falha ao carregar par\xE2metros do Ita\xFA";
    if (env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(env.BIGDATA_DB, {
          module: "calculadora",
    ...
    return json5({ ok: false, error: message, ...trace }, 500);
  }
}
async function onRequestPost12(context) {
  ...
  try {
    ...
    if (validations.length) {
      return json5({ ok: false, error: validations[0], ...trace }, 400);
    }
    const values = {
    ...
    const message = error instanceof Error ? error.message : "Falha ao salvar par\xE2metros do Ita\xFA";
    if (env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(env.BIGDATA_DB, {
          module: "calculadora",
...
var json5, resolveParametrosDb, resolveOperationalSource4;
var init_parametros = __esm({
  "api/calculadora/parametros.ts"() {
    ...
    resolveParametrosDb = /* @__PURE__ */ __name((context) => context.env.BIGDATA_DB, "resolveParametrosDb");
...
async function onRequestGet12(context) {
  const trace = createResponseTrace(context.request);
  const adminActor = resolveAdminActorFromRequest(context.request);
  const db = resolveRateLimitDb2(context);
  const source = resolveOperationalSource5(context);
  if (!db) {
    return json6({ ok: false, error: "Nenhum binding D1 dispon\xEDvel (BIGDATA_DB).", ...trace }, 503);
  }
  try {
    ...
    const message = error instanceof Error ? error.message : "Falha ao carregar painel de rate limit do Ita\xFA";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "calculadora",
    ...
    return json6({ ok: false, error: message, ...trace }, 500);
  }
}
async function onRequestPost13(context) {
  ...
  try {
    ...
    if (!routeKey) {
      return json6({ ok: false, error: "Rota de rate limit inv\xE1lida.", ...trace }, 400);
    }
    const action = String(body.action ?? "update").trim();
    if (action === "restore_default") {
      ...
      return json6({ ok: true, action: "restore_default", policies: policies2, admin_actor: adminActor, ...trace });
    }
    const enabled = body.enabled ? 1 : 0;
    const maxRequests = toPositiveInt2(body.max_requests, 2);
    const windowMinutes = toPositiveInt2(body.window_minutes, 10);
    if (maxRequests > 5e3 || windowMinutes > 1440) {
      return json6({ ok: false, error: "Par\xE2metros fora da faixa permitida.", ...trace }, 400);
    }
    await upsertRateLimitPolicy2(db, {
      routeKey,
      enabled,
    ...
    return json6({ ok: true, action: "update", policies, admin_actor: adminActor, ...trace });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Falha ao salvar painel de rate limit do Ita\xFA";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "calculadora",
...
var json6, normalizeRoute2, toPositiveInt2, resolveRateLimitDb2, resolveOperationalSource5;
var init_rate_limit2 = __esm({
  "api/calculadora/rate-limit.ts"() {
    ...
    resolveRateLimitDb2 = /* @__PURE__ */ __name((context) => context.env.BIGDATA_DB, "resolveRateLimitDb");
...
async function onRequestPost14(context) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Falha inesperada no sync do Ita\xFA";
    await finishSyncRun(env.BIGDATA_DB, {
      id: syncRunId,
      status: "error",
      finishedAt: Date.now(),
...
var parseLimit2, toHeaders4, parseObservabilidadeRows, parseRateLimitPolicies, existsObservabilidade;
...
var EXTERNAL_MEDIA_PATTERN, INTERNAL_MEDIA_PREFIX, onRequestPost15;
...
async function onRequestGet13(context) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Erro desconhecido no m\xF3dulo MainSite";
    try {
      await logModuleOperationalEvent(env.BIGDATA_DB, {
        module: "mainsite",
        source: "bigdata_db",
...
async function onRequestGet14(context) {
  ...
  try {
    ...
    if (id) {
      ...
      if (!post) {
        return buildErrorResponse3("Post n\xE3o encontrado para o ID informado.", trace, 404);
      }
    ...
    return buildErrorResponse3(message, trace, 500);
  }
}
async function onRequestPost16(context) {
  ...
  try {
    ...
    if (!title || !content) {
      return buildErrorResponse3("T\xEDtulo e conte\xFAdo s\xE3o obrigat\xF3rios para criar um post.", trace, 400);
    }
    await db.prepare(`
    ...
    return buildErrorResponse3(message, trace);
  }
}
async function onRequestPut3(context) {
  ...
  try {
    ...
    const content = parseText(body.content);
    if (!id || !title || !content) {
      return buildErrorResponse3("ID, t\xEDtulo e conte\xFAdo s\xE3o obrigat\xF3rios para atualizar um post.", trace, 400);
    }
    await db.prepare("UPDATE mainsite_posts SET title = ?, content = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(title, content, id).run();
    const row = await db.prepare(`
      SELECT id, title, content, created_at, is_pinned
      FROM mainsite_posts
      WHERE id = ?
    ...
    if (!updatedPost) {
      return buildErrorResponse3("Post n\xE3o encontrado para atualiza\xE7\xE3o.", trace, 404);
    }
    try {
      await logModuleOperationalEvent(db, {
        module: "mainsite",
...
async function onRequestDelete2(context) {
  ...
  try {
    ...
    const id = parseId(body.id);
    if (!id) {
      return buildErrorResponse3("ID v\xE1lido \xE9 obrigat\xF3rio para excluir um post.", trace, 400);
    }
...
var init_posts = __esm({
  "api/mainsite/posts.ts"() {
    ...
    buildErrorResponse3 = /* @__PURE__ */ __name((message, trace, status = 500) => new Response(JSON.stringify({
      ok: false,
      error: message,
      ...trace
    }), {
...
async function onRequestPost17(context) {
  const trace = createResponseTrace(context.request);
  try {
    const db = requireDb2(context.env);
    const body = await context.request.json();
    const adminActor = resolveAdminActorFromRequest(context.request, body);
    const id = parseId2(body.id);
    if (!id) {
      return buildErrorResponse4("ID v\xE1lido \xE9 obrigat\xF3rio para alternar fixa\xE7\xE3o do post.", trace, 400);
    }
    const current = await db.prepare("SELECT is_pinned FROM mainsite_posts WHERE id = ? LIMIT 1").bind(id).first();
    if (!current) {
      return buildErrorResponse4("Post n\xE3o encontrado para alternar fixa\xE7\xE3o.", trace, 404);
    }
    ...
    const message = error instanceof Error ? error.message : "Falha ao alternar fixa\xE7\xE3o do post do MainSite";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "mainsite",
    ...
    return buildErrorResponse4(message, trace);
  }
}
...
var init_posts_pin = __esm({
  "api/mainsite/posts-pin.ts"() {
    ...
    buildErrorResponse4 = /* @__PURE__ */ __name((message, trace, status = 500) => new Response(JSON.stringify({
      ok: false,
      error: message,
      ...trace
    }), {
    ...
    requireDb2 = /* @__PURE__ */ __name((env) => {
      if (!env.BIGDATA_DB) {
        throw new Error("BIGDATA_DB n\xE3o configurado no runtime do admin-app.");
      }
      return env.BIGDATA_DB;
...
async function onRequestPost18(context) {
  ...
  try {
    const db = context.env.BIGDATA_DB;
    if (!db) {
      return buildErrorResponse5("BIGDATA_DB n\xE3o configurado no runtime.", trace, 503);
    }
    const body = await context.request.json();
    const adminActor = resolveAdminActorFromRequest(context.request, body);
    if (!Array.isArray(body.items) || body.items.length === 0) {
      return buildErrorResponse5("Lista de itens para reordena\xE7\xE3o \xE9 obrigat\xF3ria.", trace, 400);
    }
    ...
    if (items.length === 0) {
      return buildErrorResponse5("Nenhum item v\xE1lido para reordena\xE7\xE3o.", trace, 400);
    }
    ...
    return buildErrorResponse5(message, trace);
  }
}
...
var init_posts_reorder = __esm({
  "api/mainsite/posts-reorder.ts"() {
    ...
    init_admin_actor();
    init_request_trace();
    buildErrorResponse5 = /* @__PURE__ */ __name((message, trace, status = 500) => new Response(JSON.stringify({
      ok: false,
      error: message,
      ...trace
    }), {
...
async function onRequestGet15(context) {
  ...
  try {
    ...
    return json7({ ok: true, policies, admin_actor: adminActor, ...trace });
  } catch (error) {
    ...
    return json7({ ok: false, error: message, ...trace }, 502);
  }
}
async function onRequestPost19(context) {
  ...
  try {
    ...
    if (!route) {
      return json7({ ok: false, error: "Rota de rate limit inv\xE1lida.", ...trace }, 400);
    }
    const action = String(body.action ?? "update").trim();
    ...
    if (action === "restore_default") {
      ...
      if (maxRequests === null || windowMinutes === null) {
        return json7({ ok: false, error: "Par\xE2metros de rate limit inv\xE1lidos.", ...trace }, 400);
      }
      if (maxRequests > POLICY_LIMITS.maxRequests || windowMinutes > POLICY_LIMITS.windowMinutes) {
        return json7({ ok: false, error: "Par\xE2metros fora da faixa permitida.", ...trace }, 400);
      }
      nextConfig[route] = {
    ...
    return json7({ ok: true, action, policies, admin_actor: adminActor, ...trace });
  } catch (error) {
    ...
    return json7({ ok: false, error: message, ...trace }, 500);
  }
}
var POLICY_LIMITS, POLICY_META, ROUTES, json7, toBoolean, parsePositiveInt, normalizeBucket, normalizeConfig, toClientPolicies, loadLegacyRateLimit, saveLegacyRateLimit, normalizeRoute3;
var init_rate_limit3 = __esm({
  "api/mainsite/rate-limit.ts"() {
    ...
    toClientPolicies = /* @__PURE__ */ __name((config) => ROUTES.map((route) => {
    ...
    loadLegacyRateLimit = /* @__PURE__ */ __name(async (context) => {
      const db = context.env.BIGDATA_DB;
      if (!db) {
    ...
    saveLegacyRateLimit = /* @__PURE__ */ __name(async (context, config) => {
      const db = context.env.BIGDATA_DB;
      if (!db) {
        throw new Error("BIGDATA_DB n\xE3o configurado no runtime do admin-app.");
      }
...
async function onRequestGet16(context) {
  const trace = createResponseTrace(context.request);
  try {
    const db = requireDb3(context.env);
    ...
    const message = error instanceof Error ? error.message : "Falha ao consultar settings p\xFAblicos do MainSite";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "mainsite",
    ...
    return buildErrorResponse6(message, trace, 500);
  }
}
async function onRequestPut4(context) {
  ...
  try {
    ...
    if (!isRecord(body.appearance) || !isRecord(body.rotation) || !isRecord(body.disclaimers)) {
      return buildErrorResponse6("Appearance, rotation e disclaimers precisam ser objetos JSON v\xE1lidos.", trace, 400);
    }
    const settings = {
    ...
    const message = error instanceof Error ? error.message : "Falha ao salvar settings p\xFAblicos do MainSite";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "mainsite",
    ...
    return buildErrorResponse6(message, trace);
  }
}
...
var init_settings = __esm({
  "api/mainsite/settings.ts"() {
    ...
    buildErrorResponse6 = /* @__PURE__ */ __name((message, trace, status = 500) => new Response(JSON.stringify({
      ok: false,
      error: message,
      ...trace
    }), {
    ...
    requireDb3 = /* @__PURE__ */ __name((env) => {
      if (!env.BIGDATA_DB) {
        throw new Error("BIGDATA_DB n\xE3o configurado no runtime do admin-app.");
      }
      return env.BIGDATA_DB;
...
async function onRequestPost20(context) {
  ...
  try {
    const [postsCountRow, financialCountRow, settingsRowsRaw] = await Promise.all([
      env.BIGDATA_DB.prepare("SELECT COUNT(1) AS total FROM mainsite_posts").first(),
      env.BIGDATA_DB.prepare("SELECT COUNT(1) AS total FROM mainsite_financial_logs").first(),
      env.BIGDATA_DB.prepare("SELECT id, payload FROM mainsite_settings").all()
    ]);
...
var toHeaders6, DEFAULT_SETTINGS, isValidJson;
...
var resolveToken, parseJsonOrThrow, toFirstError, cloudflareRequest, listCloudflareZones, extractDnsResult, quoteTxtContent, upsertCloudflareTxtRecord, getCloudflareDnsSnapshot;
var init_cloudflare_api = __esm({
  "api/_lib/cloudflare-api.ts"() {
    ...
    cloudflareRequest = /* @__PURE__ */ __name(async (env, path, fallback, init) => {
    ...
    listCloudflareZones = /* @__PURE__ */ __name(async (env) => {
    ...
    extractDnsResult = /* @__PURE__ */ __name(async (env, path, fallback) => {
      const result = await cloudflareRequest(env, path, fallback);
    ...
    upsertCloudflareTxtRecord = /* @__PURE__ */ __name(async (env, zoneId, name, content) => {
    ...
    getCloudflareDnsSnapshot = /* @__PURE__ */ __name(async (env, domain, zoneId) => {
...
async function onRequestPost22(context) {
  const trace = createResponseTrace(context.request);
  if (!context.env.BIGDATA_DB) {
    return toError("BIGDATA_DB n\xE3o configurado no runtime.", trace, 503);
  }
  try {
    const body = await context.request.json();
    const adminActor = resolveAdminActorFromRequest(context.request, body);
    ...
    if (!domain || !zoneId || !policyText) {
      return toError("Domain, zoneId e policyText s\xE3o obrigat\xF3rios para orquestrar o MTA-STS.", trace, 400);
    }
    ...
    await context.env.BIGDATA_DB.prepare(`
      ...
      ON CONFLICT(domain) DO UPDATE SET
        ...
        updated_at = CURRENT_TIMESTAMP
    `).bind(domain, policyText, tlsrptEmail).run();
    await context.env.BIGDATA_DB.prepare(`
    ...
    const message = error instanceof Error ? error.message : "Falha inesperada na orquestra\xE7\xE3o MTA-STS";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "mtasts",
...
var toHeaders7, normalizeDomain, generateMtastsId, toError, normalizePolicyText, normalizeTlsRptEmail;
var init_orchestrate = __esm({
  "api/mtasts/orchestrate.ts"() {
    ...
    toError = /* @__PURE__ */ __name((message, trace, status = 500) => new Response(JSON.stringify({
      ok: false,
      error: message,
      ...trace
    }), {
...
async function onRequestGet17(context) {
  ...
  const trace = createResponseTrace(request);
  const url = new URL(request.url);
  const domain = normalizeDomain2(url.searchParams.get("domain"));
  const limit = parseLimit4(url.searchParams.get("limit"));
  const avisos = [];
  if (env.BIGDATA_DB) {
    try {
      const historyRows = domain ? await env.BIGDATA_DB.prepare("SELECT gerado_em, domain, data_criacao FROM mtasts_history WHERE domain = ? ORDER BY id DESC LIMIT ?").bind(domain, limit).all() : await env.BIGDATA_DB.prepare("SELECT gerado_em, domain, data_criacao FROM mtasts_history ORDER BY id DESC LIMIT ?").bind(limit).all();
      const policyRows = domain ? await env.BIGDATA_DB.prepare("SELECT domain, policy_text, tlsrpt_email, updated_at FROM mtasts_mta_sts_policies WHERE domain = ? ORDER BY updated_at DESC LIMIT 10").bind(domain).all() : await env.BIGDATA_DB.prepare("SELECT domain, policy_text, tlsrpt_email, updated_at FROM mtasts_mta_sts_policies ORDER BY updated_at DESC").all();
      ...
      const message2 = error instanceof Error ? error.message : "Falha ao consultar bigdata_db";
...
async function onRequestGet18(context) {
  const trace = createResponseTrace(context.request);
  const url = new URL(context.request.url);
  const domain = normalizeDomain3(url.searchParams.get("domain"));
  const zoneId = String(url.searchParams.get("zoneId") ?? "").trim();
  if (!domain || !zoneId) {
    return toError2("Par\xE2metros domain e zoneId s\xE3o obrigat\xF3rios.", trace, 400);
  }
  try {
    ...
    const message = error instanceof Error ? error.message : "Falha ao carregar policy do dom\xEDnio";
    if (context.env.BIGDATA_DB) {
      try {
        await logModuleOperationalEvent(context.env.BIGDATA_DB, {
          module: "mtasts",
    ...
    return toError2(message, trace, 502);
  }
}
var normalizeDomain3, toHeaders8, toError2;
var init_policy = __esm({
  "api/mtasts/policy.ts"() {
    ...
    toError2 = /* @__PURE__ */ __name((message, trace, status = 500) => new Response(JSON.stringify({
      ok: false,
      ...trace,
      error: message
    }), {
...
async function onRequestPost23(context) {
  ...
  try {
    const [zones, historyRowsRaw, policyRowsRaw] = await Promise.all([
      ...
      env.BIGDATA_DB.prepare("SELECT gerado_em, domain FROM mtasts_history ORDER BY id DESC").all(),
      env.BIGDATA_DB.prepare("SELECT domain, policy_text, tlsrpt_email FROM mtasts_mta_sts_policies").all()
    ]);
    ...
    for (const item of dnsSnapshots) {
      ...
      await env.BIGDATA_DB.prepare(`
        ...
        ON CONFLICT(domain) DO UPDATE SET
          ...
          tlsrpt_email = excluded.tlsrpt_email,
          updated_at = CURRENT_TIMESTAMP
      `).bind(domain, policyText, item.dns.dnsTlsRptEmail ?? existing.tlsrptEmail).run();
      policiesUpserted += 1;
    }
...
async function onRequestGet19(context) {
  ...
  try {
    ...
    return toError3(message, trace, 502);
  }
}
var toHeaders10, toError3;
var init_zones = __esm({
  "api/mtasts/zones.ts"() {
    ...
    toError3 = /* @__PURE__ */ __name((message, trace, status = 500) => new Response(JSON.stringify({
      ok: false,
      ...trace,
      error: message
    }), {
...
var RSS_SOURCES, DEFAULT_MAX_ITEMS, CACHE_TTL_SECONDS, onRequestGet21;
var init_feed = __esm({
  "api/news/feed.ts"() {
    "use strict";
    init_functionsRoutes_0_834891547028554();
...
var init_excluir2 = __esm({
  "api/oraculo/excluir.ts"() {
    "use strict";
    init_functionsRoutes_0_834891547028554();
    onRequestPost24 = /* @__PURE__ */ __name(async ({ env, request }) => {
...
function parseCSV(csvText) {
  ...
  for (let i = lines.length - 1; i >= 1; i--) {
    ...
    const isIpca = tipoLower.includes("ipca") || tipoLower.includes("ntn-b") || tipoLower === "ntn-b" || tipoLower === "ntn-b principal" || pubLower.includes("ipca") || pubLower.includes("ntn-b");
    if (isIpca) {
      results.push({
        tipo: tipoTitulo,
...
var init_taxacache = __esm({
  "api/oraculo/taxacache.ts"() {
    ...
    onRequestGet24 = /* @__PURE__ */ __name(async ({ env, request }) => {
      const db = env?.BIGDATA_DB;
      if (!db || typeof db.prepare !== "function")
        return json9({ ok: false, error: "BIGDATA_DB indispon\xEDvel." }, 503);
      const url = new URL(request.url);
      ...
      try {
        ...
        if (!csvRes.ok) {
          ...
          return json9({ ok: false, error: `CSV indispon\xEDvel (HTTP ${csvRes.status}).` }, 502);
        }
       ...
       ON CONFLICT(id) DO UPDATE SET data_referencia = ?, taxa_indicativa = ?, vencimentos_json = ?, atualizado_em = ?`
        ...
        return json9({ ok: false, error: err instanceof Error ? err.message : "Erro interno." }, 500);
      }
    }, "onRequestGet");
  }
});
...
async function onRequestDelete3(context) {
  ...
  const db = env.BIGDATA_DB;
  if (!db) {
    return new Response(JSON.stringify({ ok: false, error: "Binding BIGDATA_DB indispon\xEDvel.", ...trace }), { status: 503, headers: { "Content-Type": "application/json" } });
  }
  const url = new URL(context.request.url);
  const table = url.searchParams.get("table");
  const id = url.searchParams.get("id");
  if (!table || !id) {
    return new Response(JSON.stringify({ ok: false, error: "Par\xE2metros obrigat\xF3rios: table, id.", ...trace }), { status: 400, headers: { "Content-Type": "application/json" } });
  }
  if (!ALLOWED_TABLES.includes(table)) {
    return new Response(JSON.stringify({ ok: false, error: `Tabela "${table}" n\xE3o permitida.`, ...trace }), { status: 403, headers: { "Content-Type": "application/json" } });
  }
  try {
...
async function onRequestGet26(context) {
  ...
  const db = env.BIGDATA_DB;
  if (!db) {
    return new Response(JSON.stringify({ ok: false, error: "Binding BIGDATA_DB indispon\xEDvel.", ...trace }), { status: 503, headers: headers() });
  }
  try {
    await ensureOperationalTables(db);
...
function jsonResponse(body, status = 200, additional) {
...
// ../.wrangler/tmp/pages-0gjHfO/functionsRoutes-0.834891547028554.mjs
var routes;
var init_functionsRoutes_0_834891547028554 = __esm({
  "../.wrangler/tmp/pages-0gjHfO/functionsRoutes-0.834891547028554.mjs"() {
    "use strict";
    init_transform();
    init_filename();
