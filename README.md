...
- Astrólogo e Itaú também avançaram na paridade funcional dos painéis de rate limit: detecção de alterações pendentes, restauração local (por rota/em lote) e persistência do painel completo.
...
- Auditoria textual de paridade concluída nos módulos restantes (`astrologo`, `itau`, `mainsite`): títulos de seção e CTAs foram alinhados ao vocabulário dos legados (ex.: `Câmara do Mestre`, `Arquivo Akáshico`, `Painel de controle de rate limit`, `Parâmetros vigentes`, `Sistema — settings públicos`).
