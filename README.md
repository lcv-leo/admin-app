...
- Astrólogo e Calculadora também avançaram na paridade funcional dos painéis de rate limit: detecção de alterações pendentes, restauração local (por rota/em lote) e persistência do painel completo.
- MTA-STS com fluxo de edição em modo draft: alerta de alterações não salvas para policy/TLS-RPT e restauração local do último snapshot carregado.
- Configurações globais deixaram de ser placeholder: módulo `Config` agora possui preferências operacionais locais com persistência em `localStorage`, baseline/snapshot e confirmação de alterações pendentes.
- Hubs (`apphub` e `adminhub`) agora também exibem estado de alterações não salvas e permitem restaurar snapshot local antes de persistir em D1.
- Paridade visual pixel-perfect dos hubs: preview em `card-grid` com estrutura `card` (`icon`, `title`, `desc`, `badge`) alinhada ao layout visual dos apps legados `apphub` e `adminhub`.
- Paridade semântica dos hubs: preview agora replica os títulos legados (`Portal de Apps` / `Painel Administrativo`) e o heading de catálogo com `status-dot` contextual (`Acesso Liberado` / `Ferramentas & Gestão`).
- MTA-STS recebeu chip de status operacional no cabeçalho do módulo (`sincronizado`, `alerta`, `erro`, `draft`) para replicar o feedback contínuo presente no admin legado.
- MTA-STS recebeu ajuste de microcopy para equivalência com o legado: `Orquestrador MTA-STS`, `Domínio Alvo`, `E-mail TLS-RPT (RUA)`, `Arquivo mta-sts.txt`, `REGERAR VIA MX` e CTA `Sincronizar Dados e Gerar Novo ID`.
- Refino visual adicional: `mtasts` recebeu densidade tipográfica mais próxima do legado e `hubs` ganhou microinterações de hover no preview de cards, preservando o padrão de feedback do catálogo antigo.
- Auditoria textual de paridade concluída nos módulos restantes (`astrologo`, `calculadora`, `mainsite`): títulos de seção e CTAs foram alinhados ao vocabulário dos legados (ex.: `Câmara do Mestre`, `Arquivo Akáshico`, `Painel de controle de rate limit`, `Parâmetros vigentes`, `Sistema — settings públicos`).
- Revisão visual lado a lado final concluída: no preview dos hubs, o heading de catálogo agora usa estilo legado (uppercase + tracking) e o `status-dot` passou a pulsar com cor contextual (`open`/`restricted`), reproduzindo o feedback visual dos apps originais.
