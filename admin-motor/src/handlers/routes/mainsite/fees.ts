...
export async function onRequestPost(context: FeesContext) {
  ...
  try {
    ...
    const fees: FeeConfig = {
      sumupRate:
        typeof body.sumupRate === 'number' && body.sumupRate >= 0 && body.sumupRate < 1
          ? body.sumupRate
          : DEFAULT_FEES.sumupRate,
      sumupFixed:
        typeof body.sumupFixed === 'number' && body.sumupFixed >= 0 ? body.sumupFixed : DEFAULT_FEES.sumupFixed,
      mpRate:
        typeof body.mpRate === 'number' && body.mpRate >= 0 && body.mpRate < 1 ? body.mpRate : DEFAULT_FEES.mpRate,
