...
export function parseServiceAccount(saKey: string): ServiceAccountKey {
  ...
  if (!serviceAccount.client_email || !serviceAccount.private_key) {
    throw new Error(
      'GCP_SA_KEY não contém client_email ou private_key. Verifique se o secret contém o JSON completo da service account.',
    );
  }
...
export async function getAccessToken(saKey: string): Promise<string> {
  ...
  const payload = {
    iss: sa.client_email,
    scope:
      'https://www.googleapis.com/auth/monitoring.read https://www.googleapis.com/auth/cloud-platform.read-only https://www.googleapis.com/auth/logging.read',
  ...
  try {
    ...
    throw new Error(
      'A private_key do GCP_SA_KEY está malformada. Recrie a chave JSON da service account e atualize o secret no Cloudflare.',
    );
  }
  ...
  try {
    ...
    throw new Error(
      'Falha ao importar a private_key da service account. Verifique se o secret contém uma chave PKCS#8 válida e atualizada.',
    );
  }
    ...
    return json(
      {
        ...
        setupGuide: {
          ...
          securityNote:
            'A service account terá acesso SOMENTE LEITURA a métricas. Não tem acesso a dados, billing ou escrita.',
        },
      },
      200,
    ); // 200 para o frontend poder exibir o guia
  }
