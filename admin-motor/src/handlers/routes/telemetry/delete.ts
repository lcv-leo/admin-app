...
export async function onRequestDelete(context: Context) {
  ...
  if (!table || !id) {
    return new Response(JSON.stringify({ ok: false, error: 'Parâmetros obrigatórios: table, id.', ...trace }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }
  ...
  if (!ALLOWED_TABLES.includes(table as (typeof ALLOWED_TABLES)[number])) {
    return new Response(JSON.stringify({ ok: false, error: `Tabela "${table}" não permitida.`, ...trace }), {
      status: 403,
      headers: { 'Content-Type': 'application/json' },
    });
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ ok: true, ...trace, deleted: { table, id: Number(id) } }), {
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Erro ao excluir registro.';
    return new Response(JSON.stringify({ ok: false, error: message, ...trace }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
