  ...
  return (
    <label
      style={{
        ...
        background: checked ? 'rgba(52,168,83,0.04)' : 'transparent',
      }}
    >
...
export function ModerationPanel({ showNotification }: ModerationPanelProps) {
  ...
  const handleModerate = async (id: number, status: string) => {
    ...
    try {
      ...
      showNotification('Erro ao moderar.', 'error');
    } finally {
  ...
  const handleReply = async (parentId: number) => {
    ...
    try {
      ...
      showNotification('Erro ao responder.', 'error');
    } finally {
      setActionInProgress(null);
    }
  };
          ...
          {settingsLoading ? (
            ...
            <>
              ...
              >
                ...
                <div
                  style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '4px' }}
                >
                  <Toggle
                    checked={settings.commentsEnabled}
                    onChange={(v) => setSettings((s) => ({ ...s, commentsEnabled: v }))}
                    label="Comentários habilitados"
                    hint="Permitir envio de novos comentários nos posts"
                    icon={MessageSquare}
                  />
                  <Toggle
                    checked={settings.ratingsEnabled}
                    onChange={(v) => setSettings((s) => ({ ...s, ratingsEnabled: v }))}
                    label="Avaliações habilitadas"
                    hint="Permitir que leitores avaliem os posts"
                  />
                  <Toggle
                    checked={!settings.allowAnonymous}
                    onChange={(v) => setSettings((s) => ({ ...s, allowAnonymous: !v }))}
                    label="Exigir nome"
                    hint="Tornar o campo de nome obrigatório"
                  />
                  <Toggle
                    checked={settings.requireEmail}
                    onChange={(v) => setSettings((s) => ({ ...s, requireEmail: v }))}
                    label="Exigir email"
                    hint="Tornar o campo de email obrigatório"
                    icon={Mail}
                  />
                  <Toggle
                    ...
                    hint="Todos os comentários ficam pendentes até revisão manual"
                    icon={Lock}
                    color="var(--color-warning, #f59e0b)"
                  />
                  <Toggle
                    checked={settings.notifyOnNewComment}
                    onChange={(v) => setSettings((s) => ({ ...s, notifyOnNewComment: v }))}
                    label="Notificações por email"
                    hint="Receber email a cada novo comentário"
                    icon={Mail}
                  />
                </div>
                ...
                >
                  <div className="field-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings((s) => ({ ...s, minCommentLength: Math.max(1, parseInt(e.target.value, 10) || 3) }))
                      }
                    />
                  ...
                  <div className="field-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings((s) => ({
                          ...s,
                          maxCommentLength: Math.max(10, parseInt(e.target.value, 10) || 2000),
                        }))
                      }
                    />
                  ...
                  <div className="field-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings((s) => ({
                          ...s,
                          maxNestingDepth: Math.max(0, Math.min(10, parseInt(e.target.value, 10) || 2)),
                        }))
                      }
                    />
                    <p className="field-hint">
                      Quantos níveis de respostas aninhadas são permitidos (0 = sem respostas).
                    </p>
                  </div>
                  <div className="field-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings((s) => ({
                          ...s,
                          autoCloseAfterDays: Math.max(0, parseInt(e.target.value, 10) || 0),
                        }))
                      }
                    />
                    <p className="field-hint">
                      Bloquear comentários em posts mais antigos que N dias (0 = nunca fechar).
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                  ...
                  <div className="field-group">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <input
                        ...
                        onChange={(e) =>
                          setSettings((s) => ({ ...s, autoApproveThreshold: parseFloat(e.target.value) }))
                        }
                    ...
                    <p className="field-hint">
                      Score máximo abaixo deste valor →{' '}
                      <strong style={{ color: '#34a853' }}>aprovado automaticamente</strong>.
                    </p>
                  </div>
                  <div className="field-group">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <input
                        ...
                        onChange={(e) =>
                          setSettings((s) => ({ ...s, autoRejectThreshold: parseFloat(e.target.value) }))
                        }
                    ...
                    <p className="field-hint">
                      Score acima deste valor → <strong style={{ color: '#ea4335' }}>rejeitado automaticamente</strong>.
                    </p>
                  </div>
                </div>
                ...
                <div className="field-group" style={{ marginBottom: '16px' }}>
                  ...
                  <select
                    id="mod-api-fallback"
                    value={settings.apiUnavailableBehavior}
                    onChange={(e) =>
                      setSettings((s) => ({ ...s, apiUnavailableBehavior: e.target.value as 'pending' | 'approve' }))
                    }
                  ...
                  <p className="field-hint">
                    O que acontece com comentários quando o Google Cloud NL API não responde.
                  </p>
                </div>
                ...
                <div style={{ marginBottom: '4px' }}>
                  ...
                  <p className="field-hint" style={{ marginBottom: '10px' }}>
                    Apenas as categorias selecionadas serão consideradas na análise. Categorias desmarcadas serão
                    ignoradas pelo motor de decisão.
                  </p>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            border: active
                              ? '1.5px solid var(--color-primary, #4285f4)'
              ...
              >
                ...
                >
                  <div className="field-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings((s) => ({
                          ...s,
                          rateLimitPerIpPerHour: Math.max(0, parseInt(e.target.value, 10) || 0),
                        }))
                      }
                    />
                  ...
                  <div className="field-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings((s) => ({
                          ...s,
                          duplicateWindowHours: Math.max(1, parseInt(e.target.value, 10) || 24),
                        }))
                      }
                    />
                  ...
                  <div className="field-group">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSettings((s) => ({ ...s, linkPolicy: e.target.value as 'allow' | 'pending' | 'block' }))
                      }
                    >
                ...
                <div className="field-group">
                  ...
                  />
                  <p className="field-hint">
                    Palavras e frases separadas por vírgula. Comentários contendo qualquer termo listado serão{' '}
                    <strong>rejeitados imediatamente</strong>, sem passar pela IA.
                  </p>
                </div>
              </div>
            ...
            {filterTabs.map((tab) => (
              <button
                ...
                style={{
                  ...
                  borderBottom:
                    activeFilter === tab.key ? '2px solid var(--color-primary, #4285f4)' : '2px solid transparent',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          ...
                          {comment.author_email && (
                            <span style={{ fontSize: '11px', opacity: 0.5 }}>{comment.author_email}</span>
                          )}
                          <span style={{ fontSize: '11px', opacity: 0.4, marginLeft: 'auto' }}>
                          ...
                          {activeFilter === 'pending' && (
                            <>
                              <button
                                ...
                                disabled={isBusy}
                                style={{ color: 'var(--color-success, #34a853)', fontSize: '11px', padding: '2px 8px' }}
                              >
                              ...
                              <button
                                type="button"
                                className="ghost-button"
                                onClick={() => handleModerate(comment.id, 'rejected_manual')}
                                disabled={isBusy}
                              ...
                              {comment.status === 'approved'
                                ...
                                : comment.status === 'pending'
                                  ...
                                  : comment.status === 'rejected_auto'
                                    ? 'Bloqueado pela IA'
                                    : comment.status === 'rejected_manual'
                                      ? 'Rejeitado manualmente'
                                      : comment.status}
                            </div>
                            {comment.reviewed_at && (
                              <div style={{ marginBottom: '4px' }}>
                                <strong>Revisado em:</strong> {formatDate(comment.reviewed_at)}
                              </div>
                            )}
                            {comment.admin_notes && (
                              <div style={{ marginBottom: '4px' }}>
                                <strong>Observações:</strong> {comment.admin_notes}
                              </div>
                            )}
                            {comment.moderation_decision &&
                              (() => {
                                try {
                                  const decision = JSON.parse(comment.moderation_decision) as {
                                    action: string;
                                    reason: string;
                                    maxScore: number;
                                    maxCategory: string;
                                  ...
                                  const catLabel =
                                    GCP_CATEGORIES.find((c) => c.id === decision.maxCategory)?.label ||
                                    decision.maxCategory;
                                  return (
                                    <div>
                                      <div>
                                        <strong>Decisão da IA:</strong>{' '}
                                        {decision.action === 'approved'
                                          ? 'Aprovado'
                                          : decision.action === 'rejected_auto'
                                            ? 'Rejeitado'
                                            : 'Pendente'}{' '}
                                        — {decision.reason}
                                      </div>
                                      <div>
                                        <strong>Maior score:</strong> {decision.maxScore.toFixed(2)} na categoria{' '}
                                        <em>{catLabel}</em>
                                      </div>
                                    </div>
                                  );
