...
import {
  Star, Trash2, RefreshCw, Loader2,
  Pencil, ChevronDown, ChevronUp, X, Check,
  BarChart3, Heart, Lightbulb, Sparkles, BookOpen,
  HelpCircle, Filter,
} from 'lucide-react'
    ...
    <span style={{ letterSpacing: '1px', fontSize: '14px' }} title={`${value}/5`}>
      {Array.from({ length: 5 }, (_, i) => (
        <Star
          ...
          stroke={i < value ? '#f59e0b' : 'rgba(128,128,128,0.3)'}
...
export function RatingsPanel({ showNotification }: RatingsPanelProps) {
  ...
  const handleUpdate = async (id: number) => {
    ...
    try {
      ...
      setEditingId(null)
      await fetchRatings()
    } catch { showNotification('Erro ao atualizar.', 'error') }
    finally { setActionInProgress(null) }
  }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await fetchRatings()
    } catch { showNotification('Erro na exclusão em lote.', 'error') }
    ...
    <div className="result-card" style={{ marginTop: '16px' }}>
      ...
      {stats && stats.total > 0 && (
        <div style={{
          ...
          <div style={{
            padding: '14px 16px', borderRadius: '10px',
            background: 'rgba(66,133,244,0.04)', border: '1px solid rgba(66,133,244,0.08)',
          }}>
            <div style={{ fontSize: '11px', fontWeight: 600, marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          <div style={{
            padding: '14px 16px', borderRadius: '10px',
            background: 'rgba(234,67,53,0.04)', border: '1px solid rgba(234,67,53,0.08)',
          }}>
              ...
              {Object.entries(REACTION_MAP).map(([key, { emoji, label }]) => {
                ...
                return (
                  <span key={key} style={{
                    ...
                    background: 'rgba(128,128,128,0.06)', display: 'inline-flex',
      ...
      {showFilters && (
        <div style={{
          ...
          background: 'rgba(66,133,244,0.03)', border: '1px solid rgba(66,133,244,0.08)',
          marginBottom: '12px',
        }}>
          <div className="field-group" style={{ margin: 0, flex: '0 0 auto' }}>
            <label htmlFor="ratings-filter-post" style={{ fontSize: '11px', fontWeight: 600 }}>Post ID</label>
            <input
          ...
          <div className="field-group" style={{ margin: 0, flex: '0 0 auto' }}>
            <label htmlFor="ratings-filter-stars" style={{ fontSize: '11px', fontWeight: 600 }}>Estrelas</label>
            <select
          ...
          <div className="field-group" style={{ margin: 0, flex: '0 0 auto' }}>
            <label htmlFor="ratings-filter-reaction" style={{ fontSize: '11px', fontWeight: 600 }}>Reação</label>
            <select
          ...
          {hasActiveFilters && (
            <button type="button" className="ghost-button" onClick={clearFilters} style={{ fontSize: '11px', padding: '4px 8px' }}>
              <X size={12} /> Limpar
            </button>
          )}
        </div>
      ...
      {selectedIds.size > 0 && (
        <div style={{
          ...
          borderRadius: '8px', background: 'rgba(234,67,53,0.06)', marginBottom: '12px',
          fontSize: '13px', flexWrap: 'wrap',
        }}>
          <strong>{selectedIds.size} selecionada(s)</strong>
          <button type="button" className="ghost-button" onClick={() => void handleBulkDelete()} disabled={bulkAction}
      ...
      {loading ? (
        ...
        <ul className="result-list astro-akashico-scroll" style={{ maxHeight: '500px', overflowY: 'auto' }}>
          ...
          <li style={{ padding: '6px 12px', fontSize: '12px', opacity: 0.6, cursor: 'pointer' }} onClick={toggleSelectAll}>
            <input type="checkbox" checked={selectedIds.size === ratings.length && ratings.length > 0} readOnly style={{ marginRight: '8px' }} />
          ...
          {ratings.map(record => {
            ...
            return (
              <li key={record.id} className={`post-row ${isSelected ? 'post-row--selected' : ''}`} style={{ flexDirection: 'column', alignItems: 'stretch' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                  <input type="checkbox" checked={isSelected} onChange={() => toggleSelect(record.id)} style={{ marginTop: '4px', flexShrink: 0 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', marginTop: '4px', flexWrap: 'wrap', alignItems: 'center' }}>
                      <button type="button" className="ghost-button" onClick={() => setExpandedId(isExpanded ? null : record.id)} style={{ fontSize: '11px', padding: '2px 8px' }}>
                        {isExpanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                        {isExpanded ? 'Menos' : 'Detalhes'}
                      </button>

                      <button type="button" className="ghost-button" onClick={() => startEdit(record)} disabled={isBusy} style={{ fontSize: '11px', padding: '2px 8px', color: 'var(--color-primary, #4285f4)' }}>
                        <Pencil size={12} /> Editar
                      </button>

                      <button type="button" className="ghost-button" onClick={() => void handleDelete(record.id)} disabled={isBusy} style={{ color: 'var(--color-danger, #ea4335)', fontSize: '11px', padding: '2px 8px' }}>
                    ...
                    {isExpanded && (
                      <div style={{ marginTop: '10px', padding: '10px', borderRadius: '8px', background: 'rgba(128,128,128,0.06)', fontSize: '12px' }}>
                        <div style={{ marginBottom: '4px' }}><strong>ID:</strong> {record.id}</div>
                        <div style={{ marginBottom: '4px' }}><strong>Post ID:</strong> {record.post_id}</div>
                        <div style={{ marginBottom: '4px' }}><strong>Rating:</strong> {record.rating}/5</div>
                        <div style={{ marginBottom: '4px' }}><strong>Reação:</strong> {reaction ? `${reaction.emoji} ${reaction.label}` : '—'}</div>
                        <div style={{ marginBottom: '4px' }}><strong>Voter Hash:</strong> <code style={{ fontSize: '11px', padding: '2px 4px', borderRadius: '4px', background: 'rgba(128,128,128,0.08)' }}>{maskHash(record.voter_hash)}</code></div>
                        <div><strong>Data:</strong> {formatDate(record.created_at)}</div>
                      </div>
                    )}
                    ...
                    {isEditing && (
                      <div style={{
                        ...
                        background: 'rgba(66,133,244,0.04)', border: '1px solid rgba(66,133,244,0.12)',
                      }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                          <label htmlFor={`edit-reaction-${record.id}`} style={{ fontSize: '12px', fontWeight: 500, minWidth: '60px' }}>Reação:</label>
                          <select
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          <button type="button" className="primary-button" onClick={() => void handleUpdate(record.id)} disabled={isBusy}
                            style={{ fontSize: '12px', padding: '6px 14px' }}
                          >
                            {isBusy ? <Loader2 size={12} className="spin" /> : <Check size={12} />}
                            Salvar
                          </button>
                          <button type="button" className="ghost-button" onClick={() => setEditingId(null)} style={{ fontSize: '12px', padding: '6px 14px' }}>
                            <X size={12} /> Cancelar
                          </button>
                        </div>
                      </div>
