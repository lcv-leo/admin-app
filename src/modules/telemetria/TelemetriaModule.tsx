...
export function TelemetriaModule() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ show: boolean; table: string; id: number; label: string }>({
    show: false,
    table: '',
    id: 0,
    label: '',
  ...
  const executeDelete = useCallback(async () => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`/api/telemetry/delete?table=${encodeURIComponent(table)}&id=${id}`, {
        method: 'DELETE',
      });
  ...
  const askDelete = (table: string, id: number, label: string) => setConfirmDelete({ show: true, table, id, label });
    ...
    <>
      <article className="result-card">
        <header className="result-header">
          <h4>
            <Activity size={16} /> Eventos operacionais (24 h)
          </h4>
          ...
          <ul className="result-list">
            {modules.map((m) => (
              <li key={m.module}>
                ...
                <span>
                  24h: {m.total_events} evento(s) · fallback: {m.fallback_events} · falhas: {m.error_events}
                </span>
      ...
      <article className="result-card">
        <header className="result-header">
          <h4>
            <Database size={16} /> Log de eventos (últimos 100)
          </h4>
          ...
          <ul className="result-list telemetria-event-log">
            {eventLog.map((e) => (
              <li key={e.id} className={e.ok === 0 ? 'telemetria-row-error' : ''}>
                ...
                <button
                  type="button"
                  className="telemetria-delete-btn"
                  title="Excluir evento"
                  onClick={() => askDelete('adminapp_module_events', e.id, `evento #${e.id}`)}
                >
                  <Trash2 size={13} />
                </button>
              </li>
  ...
  const renderSync = () => (
    <article className="result-card">
      <header className="result-header">
        <h4>
          <Database size={16} /> Execuções de sync
        </h4>
        ...
        <ul className="result-list">
          {sync.map((s) => (
            <li key={s.module}>
              ...
              <span>
                execuções: {s.total_runs} · sucesso: {s.success_runs} · erros: {s.error_runs}
              </span>
  ...
  const renderContatos = () => (
    <article className="result-card">
      <header className="result-header">
        <h4>
          <Mail size={16} /> Formulários de contato recebidos
        </h4>
          ...
          {contacts.map((c) => (
            <li key={c.id} className="telemetria-contact-row">
              ...
              <div className="telemetria-contact-footer">
                <span className="telemetria-date-badge">
                ...
                <button
                  ...
                  onClick={() => askDelete('mainsite_contact_logs', c.id, `contato de ${c.name}`)}
                >
                  <Trash2 size={13} />
                </button>
              </div>
  ...
  const renderShares = () => (
    <article className="result-card">
      <header className="result-header">
        <h4>
          <Share2 size={16} /> Métricas de compartilhamento
        </h4>
      ...
      {shares.length === 0 ? (
        ...
        <ul className="result-list">
          {shares.map((s) => (
            <li key={s.id}>
              <span
                className={`badge ${s.platform === 'whatsapp' ? 'badge-whatsapp' : s.platform === 'email' ? 'badge-email' : 'badge-planejado'}`}
              >
                {s.platform}
              </span>
              ...
              <button
                ...
                onClick={() => askDelete('mainsite_shares', s.id, `share #${s.id}`)}
              >
                <Trash2 size={13} />
              </button>
            </li>
  ...
  const renderChatbot = () => (
    <>
      ...
      <article className="result-card">
        <header className="result-header">
          <h4>
            <Bot size={16} /> Conversas da Consciência Auxiliar
          </h4>
        ...
        {chatLogs.length === 0 ? (
          ...
          <ul className="result-list telemetria-chat-list">
            {chatLogs.map((log) => (
              <li
                key={log.id}
                className={`telemetria-chat-row ${log.role === 'user' ? 'telemetria-chat-user' : 'telemetria-chat-bot'}`}
              >
                <div className="telemetria-chat-header">
                  <span
                    className={`telemetria-role-badge ${log.role === 'user' ? 'telemetria-role-user' : 'telemetria-role-bot'}`}
                  >
                ...
                <div className="telemetria-contact-footer">
                  ...
                  <button
                    ...
                    className="telemetria-delete-btn"
                    title="Excluir log"
                    onClick={() => askDelete('mainsite_chat_logs', log.id, `chat #${log.id}`)}
                  >
                    <Trash2 size={13} />
                  </button>
                </div>
      ...
      <article className="result-card">
        ...
        {chatAudit.length === 0 ? (
          ...
          <div className="telemetria-audit-list">
            {chatAudit.map((a) => {
              const posts = parseJsonArray<{ id?: number; title?: string; score?: number; created_at?: string }>(
                a.selected_posts_json,
              );
              ...
              return (
                <div key={a.id} className="telemetria-audit-card">
                  <div className="telemetria-audit-header">
                    ...
                    <div className="telemetria-audit-actions">
                      ...
                      <button
                        ...
                        onClick={() => askDelete('mainsite_chat_context_audit', a.id, `auditoria #${a.id}`)}
                      >
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </div>
                  <div className="telemetria-audit-question">
                  ...
                  {terms.length > 0 && (
                    <div className="telemetria-audit-terms">
                      <strong>Termos:</strong> {terms.join(', ')}
                    </div>
                  )}
                  {posts.length > 0 && (
                    <div className="telemetria-audit-posts">
                      <strong className="telemetria-audit-posts-label">
                        Publicações selecionadas ({posts.length})
                      </strong>
                      {posts.map((p, idx) => (
                        <div key={`${a.id}-${p.id || idx}`} className="telemetria-audit-post-item">
                          <div>
                            #{p.id ?? 'N/A'} — {p.title || 'Sem título'}
                          </div>
  ...
  return (
    <section className="detail-panel module-shell module-shell-telemetria">
      <div className="detail-header">
        ...
        <button
          type="button"
          className="primary-button telemetria-refresh-btn"
          ...
          <div className="admin-modal-overlay" role="dialog" aria-modal="true" aria-label="Confirmar exclusão">
            <div className="admin-modal-content">
              <button
                ...
                onClick={() => setConfirmDelete({ show: false, table: '', id: 0, label: '' })}
              >
                <X size={24} />
              </button>
              <div className="admin-modal-header">
                ...
                <p className="admin-modal-subtitle">
                  Deseja apagar permanentemente &ldquo;{confirmDelete.label}&rdquo;?
                </p>
              </div>
      ...
      {/* ── Tab content ── */}
      {loading && !data ? (
        <div className="telemetria-loading">
