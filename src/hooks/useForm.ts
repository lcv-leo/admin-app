...
export function useForm<T extends Record<string, unknown>>({
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      // Mark all as touched
      const allTouched = Object.keys(initialValuesRef.current).reduce<Record<string, boolean>>((acc, key) => {
        acc[key] = true;
        return acc;
      }, {});
...
export function useFormField({ initialValue = '', validate, onValidate }: UseFormFieldOptions): UseFormFieldReturn {
